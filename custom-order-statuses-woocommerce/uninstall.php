...
if ( is_multisite() ) {
    ...
    foreach( $cos_blog_list as $cos_blog_list_key => $cos_blog_list_value ) {
		...
		$delete_statuses = "DELETE FROM `" . $cos_multisite_prefix . "options` WHERE option_name LIKE 'alg_orders_custom_status_icon_data_%'";
		$wpdb->query( $delete_statuses );
	}
} else { 
	...
	// delete the custom order statuses
	$delete_statuses = "DELETE FROM `" . $wpdb->prefix . "options` WHERE option_name LIKE 'alg_orders_custom_status_icon_data_%'";
	$wpdb->query( $delete_statuses );
