...
class COS_Files {
	...
	public function include_files() {
		...
		COS_Lite()::include_file( 'cos-functions.php' );
		// Register Custom Post Type for custom order status.
		COS_Lite()::include_file( 'class-cos-cpt.php' );

		$tyche_files = array(
			'class-tyche-cos-tracking.php',
			'class-tyche-cos-deactivation.php',
		);
	...
	public function cos_pro_init() {
		...
		require_once COS_PLUGIN_PATH . 'includes/class-cos-migration.php';
		COS_Migration::run();

		// Load the REST API layer (router requires all controllers)
		require_once COS_PLUGIN_PATH . 'includes/api/class-cos-rest-router.php';
		COS_REST_Router::init();

		// Load the WC settings tab + asset enqueuing
		require_once COS_PLUGIN_PATH . 'includes/class-cos-settings.php';
		new COS_Settings();
	}
