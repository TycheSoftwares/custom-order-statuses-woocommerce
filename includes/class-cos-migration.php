...
class COS_Migration {
	...
	private static function migrate_license(): array {
		...
		if ( 'valid' === $old_status ) {
			$base_file       = COS_PLUGIN_PATH . 'includes/api/class-cos-rest-controller-base.php';
			$controller_file = COS_PLUGIN_PATH . 'includes/api/class-cos-rest-license-controller.php';

			if ( file_exists( $base_file ) && ! class_exists( 'COS_REST_Controller_Base' ) ) {
				require_once $base_file;
			}
			if ( file_exists( $controller_file ) && ! class_exists( 'COS_REST_License_Controller' ) ) {
				require_once $controller_file;
			}

			if ( class_exists( 'COS_REST_License_Controller' ) ) {
				$request = new WP_REST_Request( 'POST', '/cos-pro/v1/license/activate' );
				$request->set_param( 'key', $old_key );

				$controller = new COS_REST_License_Controller();
	...
	private static function seed_defaults(): void {
		$controller_file = COS_PLUGIN_PATH . 'includes/api/class-cos-rest-settings-controller.php';
		...
		if ( ! class_exists( 'COS_REST_Settings_Controller' ) ) {
			require_once $controller_file;
		}
	 ...
	 *   COS_Migration::get( 'general', 'add_to_bulk_actions', true );
	 *
