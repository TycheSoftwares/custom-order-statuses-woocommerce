...
require_once untrailingslashit( plugin_dir_path( __FILE__ ) ) . '/libraries/twilio-php/vendor/Twilio/autoload.php'; // Loads the library.
...
class COS_Hooks {
	...
	public function enqueue_script() {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] === 'wc-settings' && isset( $_GET['tab'] ) && $_GET['tab'] === 'custom-order-statuses-for-woocommerce' ) { // disabled — handled by COS_Settings

			/* wp_enqueue_script(
				'cos-react-app',
				plugin_dir_url(COS_PLUGIN_FILE) . 'build/index.js',
	...
	public function send_email_on_order_status_changed( $order_id, $status_from, $status_to, $order ) {
		...
		$alg_orders_custom_statuses_sms_enabled    = '';
		$products_with_title_link                  = array();
		$woo_statuses                              = wc_get_order_statuses();
		$alg_send_sms                              = false;
		$global_sms_statuses                       = self::cos_get_setting( 'sms', 'statuses', array() );
		$sms_content                               = '';
		...
		// Twillo info.
		$sms_from        = self::cos_get_setting( 'sms', 'from_num', '' );
		$twillo_sid      = self::cos_get_setting( 'sms', 'account_sid', '' );
		$twillo_token    = self::cos_get_setting( 'sms', 'auth_token', '' );
		$to_phone        = $order->get_billing_phone();
		$country_map     = $this->cos_country_code_map();
		$billing_country = $order->get_billing_country();
		...
		// Global SMS enabled.
		$sms_enabled = self::cos_get_yes_no( 'sms', 'enabled', false );
		if ( 'yes' === $sms_enabled ) {
			if ( in_array( 'wc-' . $status_to, $global_sms_statuses, true ) || ( empty( $global_sms_statuses ) && in_array( 'wc-' . $status_to, array_keys( $alg_orders_custom_statuses_array ), true ) ) ) {
				$alg_send_sms = true;
				$sms_content  = nl2br(
		...
		if ( ! empty( $alg_orders_custom_statuses_with_id_array ) ) {
			...
			if ( isset( $alg_orders_custom_statuses_with_id_array[ $status_to ] ) ) {
				...
				$alg_orders_custom_statuses_sms_enabled = get_post_meta( $status_post_id, 'alg_orders_custom_statuses_sms_enabled', true );
				if ( $status_post_id > 0 && 'yes' === $alg_orders_custom_statuses_sms_enabled ) {
					$alg_send_sms = true;
					if ( ! empty( get_post_meta( $status_post_id, 'alg_orders_custom_statuses_sms_content', true ) ) ) {
						$sms_content = nl2br( get_post_meta( $status_post_id, 'alg_orders_custom_statuses_sms_content', true ) );
					}
				}
			}
		}
		...
		if ( $alg_send_emails || $alg_send_sms ) {
			...
			// if product_title is in email or sms content.
			if ( strpos( $email_content, '{product_titles}' ) !== false || strpos( $sms_content, '{product_titles}' ) !== false ) {
			...
			//$sms_content = '';
			// {delivery_date}
			if ( strpos( $email_content, '{delivery_date}' ) !== false || strpos( $sms_content, '{delivery_date}' ) !== false ) {
				$delivery_date = $order->get_meta( '_orddd_delivery_date' ); // adjust meta key if needed
			}

			// {delivery_time}
			if ( strpos( $email_content, '{delivery_time}' ) !== false || strpos( $sms_content, '{delivery_time}' ) !== false ) {
				$delivery_time = $order->get_meta( '_orddd_time_slot' );
			}

			// {pickup_location}
			if ( strpos( $email_content, '{pickup_location}' ) !== false || strpos( $sms_content, '{pickup_location}' ) !== false ) {
				$pickup_location = $order->get_meta( '_pickup_location' );
			}

			// {order_type}
			if ( strpos( $email_content, '{order_type}' ) !== false || strpos( $sms_content, '{order_type}' ) !== false ) {
			...
			if ( strpos( $sms_content, '{custom_field_' ) !== false ) {
				foreach ( $order->get_meta_data() as $key => $value ) {
					$order_meta_data = $value->get_data();
					$meta_key        = $order_meta_data['key'];
					$meta_data[]     = array(
			...
			$sms_content = strip_tags( do_shortcode( str_replace( array_keys( $replaced_values ), $replaced_values, $sms_content ) ) ); // phpcs:ignore

			if ( $alg_send_sms && '' !== $sms_from ) {
				try {
					$client  = new Client( $twillo_sid, $twillo_token );
					...
					if ( $message->sid ) {
						...
						$sms_msg         = __( 'Custom SMS sent.', 'custom-order-statuses' );

						$order->add_order_note( $sms_msg );
					}
				} catch ( Exception $e ) {
					$msg = $e->getMessage();
